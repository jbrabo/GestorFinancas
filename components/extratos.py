...
def imprimir_tabela (data):
    dfExtrato = pd.DataFrame(data)
    dfExtrato['Data'] = pd.to_datetime(dfExtrato['Data']).dt.date

    dfExtrato = dfExtrato.fillna('-')
 

    dfExtrato.sort_values(by='Data', ascending=False)
    ...
    tabela = dash_table.DataTable(
        ...
        columns=[
            ...
            for i in dfExtrato.columns
        ],

        data=dfExtrato.to_dict('records'),
...
def bar_chart(data, theme):
    dfExtrato = pd.DataFrame(data)   
    dfExtrato_grouped = dfExtrato.groupby("Categoria").sum()[["Valor"]].reset_index()
    graph = px.bar(dfExtrato_grouped, x='Categoria', y='Valor', title="Despesas Gerais")
