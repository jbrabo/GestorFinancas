                    ...
                    dbc.ModalBody([
                        ...
                        dbc.Row([
                            ...
                            dbc.Col([
                                ...
                                dbc.Select(id="select_receita", options=sorted([{"label": i, "value": i} for i in cat_receita], key = lambda x: x['value']), value=cat_receita[0])
                            ], width=4)
                ...
                dbc.ModalBody([
                    ...
                    dbc.Row([
                        ...
                        dbc.Col([
                            ...
                            dbc.Select(id="select_despesa", options=sorted([{"label": i, "value": i} for i in cat_despesa], key = lambda x: x['value']), value=cat_despesa[0])
                        ], width=4)
                    ], style={"margin-top": "25px"}),
...
def salve_form_receita(n, descricao, valor, date, switches, categoria, dict_receitas):
    ...
    if n and not(valor == "" or valor== None):
        ...
        recebido = 'Recebido' if 1 in switches else 'Pendente'
        fixo = 'Recorrente' if 2 in switches else 'Única'

        df_receitas.loc[df_receitas.shape[0]] = [valor, date, categoria, descricao, recebido, fixo]
...
def salve_form_despesa(n, descricao, valor, date, switches, categoria, dict_despesas):
    ...
    if n and not(valor == "" or valor== None):
        ...
        recebido = 'Pago' if 1 in switches else 'Pendente'
        fixo = 'Recorrente' if 2 in switches else 'Única'
...
def imprimir_tabela (data):
    ...
    tabela = dash_table.DataTable(
        ...
        columns=[
            ...
            if i == "Descrição" or i == "Periódica" or i == "Status"
...
@app.callback(
    ...
    Input("datatable-despesa", "data"),
    prevent_initial_call=True,
)

def remover_despesa(tabela):

    dff = pd.DataFrame(tabela)        
    dff.to_csv("df_despesas.csv")
    dff = dff.to_dict()
    return dff
...
@app.callback(
    ...
    Input("datatable-receita", "data"),
    prevent_initial_call=True,
)

def remover_receita(tabela):

    dff = pd.DataFrame(tabela)        
    dff.to_csv("df_receitas.csv")
    dff = dff.to_dict()
    return dff
